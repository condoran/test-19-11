            ...
            <Box sx={{margin: "40px 0", border: "1px solid white", borderRadius: "5px", paddingBottom: "50px", position: "relative"}}>
                {gists.length > 0 ? (gists.map((el) => {
                    return Object.values(el?.files).length > 0 && Object.values(el?.files)?.map((elem) => {return (
                        <Box onClick={() => {window.open(elem?.raw_url, '_blank', 'noopener,noreferrer');}} sx={{display: "grid", gridTemplateColumns: "1fr 35px 100px", cursor: "pointer", padding: "10px", gap: "20px", borderBottom: "1px solid white" }} key={el?.id}>
                ...
                <Box sx={{position: "absolute", bottom: "7px", right: "10px", display: "flex"}}>
                    ...
                    <Button sx={{color: "white"}} onClick={() => {setPage(parseInt(page) - 1)}}>{"<"}</Button>
                    <Button  sx={{color: "white"}} onClick={() => {setPage(parseInt(page) + 1)}}>{">"}</Button>
                </Box>
            </Box>
        </Box>
